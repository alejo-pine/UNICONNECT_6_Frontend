...
export const forumHttpService = {
  ...
  async createQuestion(
    ...
    try {
      ...
      const json = await readJson(response);
      if (response.status === 403) {
        return { success: false, error: 'No tienes permisos para publicar en este foro.', isForbidden: true };
      }
      if (!response.ok) {
        return { success: false, error: getErrorMessage(json, response.status) };
      }
  ...
  async createAnswer(
    ...
    try {
      ...
      const json = await readJson(response);
      if (response.status === 403) {
        return { success: false, error: 'No tienes permisos para responder en este foro.', isForbidden: true };
      }
      if (!response.ok) {
        return { success: false, error: getErrorMessage(json, response.status) };
      }
  ...
  async acceptAnswer(
    ...
    try {
      ...
      const json = await readJson(response);
      if (response.status === 403) {
        return { success: false, error: 'Solo los docentes pueden aceptar respuestas.', isForbidden: true };
      }
      if (!response.ok) {
        return { success: false, error: getErrorMessage(json, response.status) };
      }
